...
|---|---|---|
| `horizon.sh` | **the coordination horizon.** Distance vs discovery vs sustained exploitation. `--ants` sweeps colony size, `--evap` the decay rate | **current — this is the main instrument** |
| `traffic.sh` | **is colony size interchangeable with trail strength?** Matched-traffic arms: a big quiet colony against a small loud one, laying the same pheromone mass per tick | **current** |
...
| `scaling.py` | **the break-even colony size N\*(D) and its exponent.** Pure-stdlib reader over a `horizon.sh --json` dump; needs no toolchain | **current** |
| `summarize.sh` | a `.run.json` game record → 8 KB interpreted summary, decoded through stigsim's own vocabulary | current; use for anything in `games-records/` |
